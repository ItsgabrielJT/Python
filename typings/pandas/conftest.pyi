...
@pytest.fixture(params=[None, "gzip", "bz2", "zip", "xz", pytest.param("zstd", marks=td.skip_if_no("zstandard"))])
...
@pytest.fixture(params=["gzip", "bz2", "zip", "xz", pytest.param("zstd", marks=td.skip_if_no("zstandard"))])
...
@pytest.fixture
def multiindex_year_month_day_dataframe_random_data(): # -> DataFrame | Series* | Series | Any:
    """
...
index_flat2 = ...
@pytest.fixture(params=[key for key in indices_dict if notisinstance(indices_dict[key], MultiIndex) and indices_dict[key].is_unique])
...
@pytest.fixture(params=[key for key in indices_dict if not(key in ["int", "uint", "range", "empty", "repeats"] or key.startswith("num_")) and notisinstance(indices_dict[key], MultiIndex)])
...
@pytest.fixture
def iris(datapath): # -> TextFileReader:
    """
...
@pytest.fixture
def fixed_now_ts(): # -> Timestamp | NaTType:
    """
...
def any_skipna_inferred_dtype(request): # -> tuple[Unknown, NDArray[Any]]:
    """
