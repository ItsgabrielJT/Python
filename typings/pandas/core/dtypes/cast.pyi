...
def maybe_infer_dtype_type(element): # -> dtype[Unknown] | None:
    """
...
def coerce_indexer_dtype(indexer, categories): # -> NDArray[int8] | NDArray[int16] | NDArray[int32] | NDArray[int64]:
...
def astype_float_to_int_nansafe(values: np.ndarray, dtype: np.dtype, copy: bool) -> np.ndarray:
    """
...
def astype_array_safe(values: ArrayLike, dtype, copy: bool = ..., errors: str = ...) -> ArrayLike:
    """
    Cast array (ndarray or ExtensionArray) to the new dtype.
...
def maybe_unbox_datetimelike_tz_deprecation(value: Scalar, dtype: DtypeObj): # -> str | int | float | bool | Period | Timestamp | Timedelta:
    """
